...
export function getRelatedPuzzlesForLesson(lesson: MakrukLesson, limit = 3): Puzzle[] {
  const scored: Array<{ puzzle: Puzzle; score: number }> = [];
  for (const puzzle of PUZZLES) {
    const score = countConceptHits(puzzle, lesson);
    if (score > 0) {
      scored.push({ puzzle, score });
    }
  }
  ...
  return scored.slice(0, limit).map(entry => entry.puzzle);
}
