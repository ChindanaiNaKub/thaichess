...
const EDITORIAL_LIVE_OVERRIDES = new Map<number, EditorialLiveOverride>([
  ...
  [9009, {
    ...
    tags: ['editorial-live', 'reviewed-practical', 'double-attack', 'overload'],
    streakTier: 'practical_attack',
  }],
  [9011, {
  ...
  [9020, {
    ...
    tags: ['editorial-live', 'reviewed-practical', 'pin', 'conversion'],
    streakTier: 'forcing_conversion',
  }],
  [9023, {
    ...
    tags: ['editorial-live', 'reviewed-practical', 'hanging-piece', 'forcing-sequence'],
    streakTier: 'forcing_conversion',
  }],
]);
...
const MANUAL_REVIEW_DRAFTS: ManualReviewDraft[] = [
  {
    id: 9199,
    title: 'Imported Black Conversion Candidate: Rook Takes Ma with Check',
    description: 'Black to move. Win material in 1 with the checking rook capture on f3.',
    explanation: 'Imported from the same board image for manual review. The clean practical conversion is Rxf3+, which wins the knight immediately while keeping the white king tied down.',
    ...
    acceptedMoves: [
      {
        move: candidateMove('g3', 'f3'),
        lineId: 'main',
        explanation: 'Rxf3+ wins the knight immediately while keeping the initiative with check.',
      },
    ],
    solutionLines: [
      {
        id: 'main',
        label: 'Checking rook capture',
        moves: candidateLine('g3-f3'),
        outcome: {
          result: 'black-win',
          reason: 'material_win',
          explanation: 'Black wins the knight immediately with check, preserving the cleanest practical conversion from the imported position.',
        },
      },
    ],
    solution: candidateLine('g3-f3'),
    hint1: 'Look for the move that wins material with check.',
    hint2: 'The cleanest answer takes the knight on f3 with the rook, not the met.',
    keyIdea: 'When two captures win a piece, prefer the one that keeps the initiative with check.',
    commonWrongMove: candidateMove('e4', 'f3'),
    wrongMoveExplanation: 'Met takes ma also wins the knight, but it gives up the stronger checking capture and lets White regroup more easily.',
    takeaway: 'In practical Makruk tactics, the checking capture is often stronger than an equal non-checking capture.',
    toMove: 'black',
    sideToMove: 'black',
    ...
    tags: ['image-import', 'candidate-from-photo', 'conversion-candidate', 'checking-capture', 'needs-line-review'],
    origin: 'curated-manual',
  },
  {
    id: 9200,
    title: 'Imported Black Follow-Up Draft: Rook Takes Ma with Check',
    description: 'Black to move. Find the checking rook capture that wins the knight cleanly from the photo position.',
    explanation: 'Imported from the same board image for manual review. This alternate draft records the cleaner practical conversion: Rxf3+ wins the knight immediately and forces White to answer the check.',
    ...
    objective: 'Black to move. Win a full knight with the checking rook capture on f3.',
    whyPositionMatters: 'This alternate draft keeps the same user-supplied board image but stores the cleaner practical interpretation for later editorial dedupe and review.',
    dependsOnCounting: false,
    ruleImpact: 'No counting issue is part of the image intake yet. This draft is quarantined until the practical conversion is reviewed against Makruk movement and deduped against sibling imports.',
    goal: {
      kind: 'material-win',
      result: 'black-win',
      reason: 'material_win',
