...
const EDITORIAL_MANUAL_DRAFTS: EditorialLiveManualDraft[] = [
  ...
  {
    id: 9104,
    title: 'Take the Ma, Net the Khun',
    description: 'White to move. Start with the bia-ngai capture that checks, then let the Ma pair finish the net.',
    explanation: 'Bxg3+ is stronger than a loose Ma check because it removes the black Ma and forces Black into a cramped king net. If Black blocks with the f-pawn, Nf5# ends it at once. If the Khun captures on g3, Nf5+ drives it to f3 and Ng5# finishes.',
    source: 'Facebook group: คุยหมากรุกกับเซียนบอล · post "ตาขาวเดินชนะ"',
    sourceLicense: 'ดัดแปลงเฉพาะตำแหน่งเพื่อการศึกษา · ไม่นำภาพต้นฉบับมาใช้',
    sourceAuthor: 'คุยหมากรุกกับเซียนบอล',
    sourcePermissionStatus: 'position-only',
    theme: 'MateIn3',
    motif: 'Pawn capture clears the Ma net',
    difficulty: 'advanced',
    difficultyScore: 1780,
    streakTier: 'mate_pressure',
    objective: 'Checkmate Black by taking the Ma on g3 with check and preserving the knight net.',
    whyPositionMatters: 'This is a practical Makruk attacking puzzle adapted from a Facebook community board post: White must see that the pawn capture is not just material, it is the move that clears the mating net.',
    dependsOnCounting: false,
    ruleImpact: 'No counting issue applies. The win is decided by check, forced replies, and the two Ma coordinating around the exposed Khun.',
    goal: {
      kind: 'checkmate',
      result: 'white-win',
      reason: 'checkmate',
    ...
    acceptedMoves: [
      {
        move: candidateMove('h2', 'g3'),
        lineId: 'pawn-block',
        explanation: 'Bxg3+ removes the black Ma with check and forces Black into a mating net.',
      },
    ],
    solutionLines: [
      {
        id: 'pawn-block',
        label: 'Pawn recapture fails immediately',
        moves: candidateLine('h2-g3', 'f4-g3', 'e7-f5'),
        outcome: {
          result: 'white-win',
          reason: 'checkmate',
          explanation: 'If Black answers by recapturing with the f-pawn, Nf5# lands immediately.',
        },
      },
      {
        id: 'king-capture',
        label: 'Khun capture walks into the Ma net',
        moves: candidateLine('h2-g3', 'h4-g3', 'e7-f5', 'g3-f3', 'e6-g5'),
        outcome: {
          result: 'white-win',
          reason: 'checkmate',
          explanation: 'If the Khun captures on g3, Nf5+ forces Kf3 and the second Ma finishes with Ng5#.',
        },
      },
    ],
    solution: candidateLine('h2-g3', 'h4-g3', 'e7-f5', 'g3-f3', 'e6-g5'),
    commonWrongMove: candidateMove('e7', 'f5'),
    hint1: 'Do not start with the Ma check. First remove the black Ma with check.',
    hint2: 'After Bxg3+, both black replies leave the Khun trapped by the Ma pair.',
    keyIdea: 'The bia-ngai capture is a forcing clearance move: it wins material, gives check, and opens the exact squares the two Ma need for mate.',
    wrongMoveExplanation: 'Nf5+ is tempting because it checks right away, but it leaves the black Ma on g3 and gives Black more ways to untangle.',
    takeaway: 'When a bia-ngai capture gives check and removes a key defender, calculate the king net before rushing to the obvious Ma check.',
    toMove: 'white',
    sideToMove: 'white',
    board: candidateBoard(
      ['g1', 'K', 'white'],
    ...
    tags: ['editorial-live', 'reviewed-practical', 'mate-preparation', 'ma-net', 'branching-mate', 'forcing-check', 'image-import'],
  },
  {
    id: 9105,
    title: 'Ruea to the Back Rank',
    description: 'White to move. The tempting capture wins material, but the back-rank ruea move wins the game.',
    explanation: 'Rg8# works because the black Khun on c8 is boxed in by the back rank and White’s supporting pieces cover the escape squares. Grabbing the rook on a5 is material, but it misses the immediate mate.',
    source: 'User-supplied Facebook Makruk board image · post "ตาขาวเดินชนะ"',
    sourceLicense: 'ดัดแปลงเฉพาะตำแหน่งเพื่อการศึกษา · ไม่นำภาพต้นฉบับมาใช้',
    sourceAuthor: null,
    sourcePermissionStatus: 'position-only',
    theme: 'MateIn1',
    motif: 'Back-rank ruea mate',
    difficulty: 'intermediate',
    difficultyScore: 1260,
    streakTier: 'mate_pressure',
    objective: 'White to move. Checkmate by lifting the ruea to the back rank.',
    whyPositionMatters: 'Black has loose material on a5, but the Khun on c8 has no room if the ruea reaches g8 with check.',
    dependsOnCounting: false,
    ruleImpact: 'No counting issue applies. This is a direct checkmate pattern: the back-rank line matters more than material.',
    goal: {
      kind: 'checkmate',
      result: 'white-win',
      reason: 'checkmate',
    ...
    acceptedMoves: [
      {
        move: candidateMove('g7', 'g8'),
        lineId: 'main',
        explanation: 'Rg8# attacks along the eighth rank while the support pieces cover the Khun’s flight squares.',
      },
    ],
    solutionLines: [
      {
        id: 'main',
        label: 'Back-rank ruea mate',
        moves: candidateLine('g7-g8'),
        outcome: {
          result: 'white-win',
          reason: 'checkmate',
          explanation: 'The ruea check from g8 leaves the black Khun with no legal capture, block, or escape.',
        },
      },
    ],
    solution: candidateLine('g7-g8'),
    commonWrongMove: candidateMove('b6', 'a5'),
    hint1: 'Do not start by collecting the loose ruea on a5.',
    hint2: 'Put your ruea on the eighth rank so it checks through the black Khun’s whole row.',
    keyIdea: 'The immediate mate is stronger than the material grab: the supporting pieces already seal b7, c7, and d7.',
    wrongMoveExplanation: 'Bxa5 wins a rook, but it gives Black a move. The position has a direct mate available right now.',
    takeaway: 'When the enemy Khun is trapped on the back rank, checkmate comes before material.',
    toMove: 'white',
    sideToMove: 'white',
    board: candidateBoard(
      ['c1', 'K', 'white'],
    ...
    tags: ['editorial-live', 'reviewed-practical', 'mate-in-one', 'back-rank-mate', 'ruea-mate', 'material-vs-mate', 'image-import'],
  },
  {
    id: 9106,
    title: 'Ma Check, Close the Score',
    description: 'White to move. The fastest finish starts with the Ma check, not the obvious ruea check.',
    explanation: 'Na6-c5+ is the engine-confirmed forcing start. After the Khun steps to c7, the second Ma joins from c2-b4 and White keeps forcing until the ruea lands on c7 for mate.',
    source: 'User-supplied Facebook Makruk board image · prompt "ทีขาวเดิน จะปิดสกอร์อย่างไรให้เร็วที่สุด"',
    sourceLicense: 'ดัดแปลงเฉพาะตำแหน่งเพื่อการศึกษา · ไม่นำภาพต้นฉบับมาใช้',
    sourceAuthor: null,
    sourcePermissionStatus: 'position-only',
    theme: 'MateIn3',
    motif: 'Ma check starts a long forced mate',
    difficulty: 'advanced',
    difficultyScore: 2140,
    streakTier: 'mate_pressure',
    verification: {
      engineSource: 'binary',
      searchDepth: 16,
      searchNodes: 1718233,
      multiPvGap: 700,
      onlyMoveChainLength: 15,
      countCriticality: 'none',
      verificationStatus: 'engine_verified',
    },
    objective: 'White to move. Start with the Ma check and keep the forcing line until the ruea mates on c7.',
    whyPositionMatters: 'This imported position is a practical speed-finishing puzzle: the flashy ruea check is slower, while the Ma check fixes the Khun route and starts the forced net.',
    dependsOnCounting: false,
    ruleImpact: 'No counting issue applies. The result is decided by a forced checking net around the black Khun.',
    goal: {
      kind: 'checkmate',
      result: 'white-win',
      reason: 'checkmate',
    ...
    acceptedMoves: [
      {
        move: candidateMove('a6', 'c5'),
        lineId: 'main',
        explanation: 'Nc5+ forces the Khun into the net and lets the second Ma join before the ruea finishes.',
      },
    ],
    solutionLines: [
      {
        id: 'main',
        label: 'Ma check starts the fastest mate',
        moves: candidateLine(
          'a6-c5',
          'b7-c7',
          'c2-b4',
        ...
        outcome: {
          result: 'white-win',
          reason: 'checkmate',
          explanation: 'The Ma checks pull the Khun into c-file pressure, and the final Rc7# ends the game.',
        },
      },
    ],
    solution: candidateLine(
      'a6-c5',
      'b7-c7',
      'c2-b4',
      'h2-d2',
    ...
    commonWrongMove: candidateMove('f8', 'b8'),
    hint1: 'The ruea check looks natural, but the fastest finish starts with a Ma check.',
    hint2: 'After Nc5+, bring the other Ma into the net before the ruea delivers the final mate.',
    keyIdea: 'When the Khun has one narrow path, a Ma check can decide the route before the ruea closes the file.',
    wrongMoveExplanation: 'Rb8+ checks, but it lets the Khun step away without entering the forced mate net. The Ma check is more exact.',
    takeaway: 'In Makruk mating attacks, the fastest finish is often the move that fixes the Khun first, not the loudest ruea check.',
    toMove: 'white',
    sideToMove: 'white',
    board: candidateBoard(
      ['d3', 'K', 'white'],
    ...
    tags: ['editorial-live', 'reviewed-practical', 'mate-preparation', 'ma-net', 'long-forced-mate', 'forcing-check', 'image-import'],
  },
  {
    id: 9107,
    title: 'Quiet Ruea Net Before Ma Mate',
    description: 'White to move. The quickest win is the quiet ruea net, not the immediate Ma check.',
    explanation: 'Rh7-b7 is a quiet mate threat that shuts the Khun in. Black can try to give the Khun air with ...Na4-c3, but Ne6-c7# still finishes immediately.',
    source: 'Facebook group: หมากรุกไทย อ.ไพโรจน์ สุวรรณ์ · post "ตาขาวเดินชนะ!!"',
    sourceLicense: 'ดัดแปลงเฉพาะตำแหน่งเพื่อการศึกษา · ไม่นำภาพต้นฉบับมาใช้',
    sourceAuthor: 'หมากรุกไทย อ.ไพโรจน์ สุวรรณ์',
    sourcePermissionStatus: 'position-only',
    theme: 'MateIn2',
    motif: 'Quiet ruea net threatens unavoidable ma mate',
    difficulty: 'advanced',
    difficultyScore: 1820,
    streakTier: 'mate_pressure',
    verification: {
      engineSource: 'binary',
      searchDepth: 102,
    ...
    objective: 'White to move. Play the quiet ruea net that leaves Black unable to stop Nc7 mate.',
    whyPositionMatters: 'This Facebook group position is a strong Makruk lesson because the winning move is not a capture or an immediate check. The ruea move first closes the Khun’s route, then the Ma finishes.',
    dependsOnCounting: false,
    ruleImpact: 'No counting issue applies. The win is decided by a forced mating net around the exposed black Khun.',
    goal: {
      kind: 'checkmate',
      result: 'white-win',
      reason: 'checkmate',
    ...
    acceptedMoves: [
      {
        move: candidateMove('h7', 'b7'),
        lineId: 'main',
        explanation: 'Rb7! creates an unavoidable Nc7 mate while keeping the black Khun boxed on the a-file.',
      },
    ],
    solutionLines: [
      {
        id: 'main',
        label: 'Quiet ruea net, then Ma mate',
        moves: candidateLine('h7-b7', 'a4-c3', 'e6-c7'),
        outcome: {
          result: 'white-win',
          reason: 'checkmate',
          explanation: 'The black Ma move cannot open enough space; Nc7# covers the escape squares and ends the game.',
        },
      },
    ],
    solution: candidateLine('h7-b7', 'a4-c3', 'e6-c7'),
    commonWrongMove: candidateMove('e6', 'c7'),
    hint1: 'The Ma check is tempting, but first ask whether Black can be denied every useful reply.',
    hint2: 'Move the ruea sideways to b7 so the next Ma jump becomes unstoppable.',
    keyIdea: 'A quiet ruea move can be stronger than a check when it builds a one-move mate threat the defender cannot parry.',
    wrongMoveExplanation: 'Nc7+ is a natural check, but it is slower. The engine-confirmed quickest finish starts by sealing the net with Rb7.',
    takeaway: 'In Makruk mating nets, do not rush the first check if a quiet ruea move creates an unavoidable mate.',
    toMove: 'white',
    sideToMove: 'white',
    board: candidateBoard(
      ['c6', 'K', 'white'],
    ...
    tags: ['editorial-live', 'reviewed-practical', 'mate-preparation', 'quiet-move', 'ruea-net', 'ma-mate', 'image-import'],
  },
];
...
const MANUAL_REVIEW_DRAFTS: ManualReviewDraft[] = [
  {
    ...
    objective: 'Black to move. Win a full knight with the checking rook capture on f3.',
    whyPositionMatters: 'This draft comes from the same user-supplied board image and reframes the practical question around the cleanest conversion: Black should take on f3 with check instead of grabbing with the met first.',
    dependsOnCounting: false,
